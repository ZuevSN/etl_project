...
class AppConfig:
    ...
    def _build_protected(self, protected):
        ...
        if bad_keys:
            self.logger.warning(f'Не найдены защищенные параметры конфигурации: {bad_keys}')
    ...
    def _load_config(self):
        self.logger.info(f'Начата сборка конфигурации')
        """Загружаю настройки из основного приложения (по умолчанию)"""
        self._params.update(self._defaults)
       
        self.logger.debug(f'В параметры конфигурации добавлены значения по умолчанию')
        ...
        if Path(self._params.get('ENV_FILE')).exists():
            ...
            self.logger.debug(f'В параметры конфигурации добавлены значения из файла настроек')
        else:
            self.logger.warning(f"Не обнаружен файл настроек по пути {self._params.get('ENV_FILE')}. Будут использованы значения по умолчанию")
        self.logger.debug(f'Параметры конфигурации: {list(self._params.keys())}')
        self.logger.info(f'Конфигурация подготовлена.')
        self.logger.info(f'Программа выполняется')
        self.logger.setLevel(self._params['LEVEL_LOG'])
    def set(self, key: str, value: Any):
        if key in self._protected:
            self.logger.warning(f"Попытка изменить защищиенный параметр конфигурации{key}. За вами выехали")
            return
